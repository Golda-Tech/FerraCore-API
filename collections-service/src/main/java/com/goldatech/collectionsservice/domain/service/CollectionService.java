    ...
    public Mono<CollectionResponse> initiateCollection(InitiateCollectionRequest request) {
        ...
        return Mono.fromCallable(() -> collectionRepository.save(newCollection))
                .flatMap(savedCollection -> {
                    // 3. Prepare request for the external payment API
                    ExternalPaymentApiRequest externalApiRequest = new ExternalPaymentApiRequest(
                            request.amount(),
                            request.currency(),
                            request.customerId(),
                            savedCollection.getCollectionRef(), // Use our internal ref as clientTransactionId for callback mapping
                            request.description(),
                            serviceCallbackUrl // Our service's callback URL
                    );

                    // 4. Call the external payment gateway
                    return externalPaymentGatewayService.initiatePayment(externalApiRequest)
                            .flatMap(externalApiResponse -> {
                                // 5. Update our collection record with external reference and status
                                savedCollection.setExternalRef(externalApiResponse.externalTransactionId());
        ...
        return Mono.fromCallable(() -> collectionRepository.findByCollectionRef(callbackRequest.transactionId()))
                .flatMap(optionalCollection -> {
                    if (optionalCollection.isEmpty()) {
                        ...
                        return Mono.error(new IllegalArgumentException("Collection not found."));
                    }
